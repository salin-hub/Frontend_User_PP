...
const Category = () => {
    ...
    const fetchCategoryDetails = async () => {
        try {
            setLoading(true);
            const response = await axios_api.get(`/categories/${id}/books`);
            setCategory(response.data || { books: [] }); // Ensure category always has a 'books' array
        } catch (err) {
        ...
        <>
            ...
            <div className="Books">
                <div className="Name_menu">
                    <h1>{category.category || 'Category Name'}</h1>
                </div>
            </div>
